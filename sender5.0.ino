...
void loop() {
    ...
    if (millis() - lastSend > 1500) {
        ...
        Serial.printf("Sent -> Acc: %.2f | GyroX: %.2f | Alt: %.2f | Pres: %.2f | Lat: %.6f | Lon: %.6f | Spd: %.2f | Sat: %d\n", 
                      myData.totalAccel, myData.gyroX, myData.altitude, myData.pressure, myData.lat, myData.lng, myData.speed, myData.satellites);
    }
}